...
namespace FsmModule
{
    public class Fsm<T>
    {
        ...
        public void Start(T stateId)
        {
            ...
            {
                this.LogError("State not found");
                return;
            }
            currentId = stateId;
            nextState.Enter();
            if(!HasEvent(EStateActionType.Start, out string eventName)) return;
            MessageSystem.Instance.Publish<IFsmMessageHandlerType, IFsmMessage<T>>(new IFsmMessage<T>(eventName, stateId));
        }
